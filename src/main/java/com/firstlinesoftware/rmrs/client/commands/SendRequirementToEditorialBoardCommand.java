...
public class SendRequirementToEditorialBoardCommand extends BrowserCommand<DTO> {
    ...
    public Widget getInstance(final Browser<DTO> view) {
        button = createButton(messages.sendToEditorialBoard(), new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                ...
                StandardPopups.enterValue(new AutoLayoutTextArea(), messages.comment(), null, button, new SuccessCallback<String>() {
                    ...
                    public void onSuccess(String comment) {
                        ...
                        requirementProxy.setLifecycle(AbstractRoute.LIFECYCLE_ONSIGNING, ids, comment, new VoidUserActionCallback(
    ...
    protected boolean isEnabled(Browser<DTO> view) {
        ...
        if(selected != null && !selected.isEmpty()) {
            for (DTO s : selected) {
                if(!(s instanceof Requirement && AbstractRoute.LIFECYCLE_APPROVED.equals(((Requirement) s).lifecycle))) {
                    return false;
                }
            }
            return true;
